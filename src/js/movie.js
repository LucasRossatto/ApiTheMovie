...
const fetchAndDisplayData = async () => {
  try {
    const response = await fetch(`${Main_url}/movie/${id}?api_key=${API_Key}&language=pt-BR`);
    if (!response.ok) {
...
const createCard = (item) => {
  ...
  const backDropContainer = document.querySelector(".backGroundImg"); // Corrigido para selecionar a classe
  card.className = "article-wrapper";
  const BackdropUrl = `https://image.tmdb.org/t/p/w500${item.backdrop_path}`; // Corrigido para `backdrop_path`
